...
class LoRaRadio:
    def __init__(self, port: str, device_addr: str, baudrate: int = 9600):
        """
        Initialize the LoRa radio interface

        :param port: the serial port the LoRa radio is connected to.
    ...
    def open(self):
        """
        Opens the LoRa radio serial connectoin.
        """
        self.serial = serial.Serial(self.port, self.baudrate, timeout=1)
        print(f"LoRa radio opened on {self.port} with baudrate {self.baudrate}")

    def close(self):
        """
        Closes the LoRa radio serial connection.
        """
        if self.serial and self.serial.is_open:
            self.serial.close()
        print("LoRa radio closed")
